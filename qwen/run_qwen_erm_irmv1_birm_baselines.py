...
def evaluate_classifier(model, head, method_name, max_batches=None):
    ...
    with torch.no_grad():
        for i, val_b in enumerate(val_loader):
            ...
            val_logits = head(h_val) if method_name in ['erm', 'irmv1'] else head.predict_logits(h_val)
...
def run_experiment(method_name, model, max_steps=1000): 
    ...
    if method_name in ['erm', 'irmv1', 'birm']:
...
# ==========================================
# 5. 依次执行三大基线算法 (冻结 Qwen 底座 + 训练分类头)
# ==========================================
MAX_STEPS = 600 
results = {}

print("\n" + "*"*75)
print("⭐ Qwen 基线实验：BIRM / ERM / IRMv1")
print("*"*75)
...
# ==========================================
print("\n" + "="*80)
print("🏆 Qwen 基线实验结果汇总 (对抗环境 + 全量反转测试) 🏆")
print("="*80)
...
pd.DataFrame(
    [
        {"method": name, "runtime_min": value[0], "reversed_test_acc": value[1]}
        for name, value in results.items()
    ]
).to_csv("/root/qwen_erm_irmv1_birm_baseline_results.csv", index=False)
print("📄 已保存: /root/qwen_erm_irmv1_birm_baseline_results.csv")
