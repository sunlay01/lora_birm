...
def main():
    config = ExperimentConfig(
        ...
        output_prefix="official_cifarmnist_notebook_peak_multiseed",
        irm_base_steps=2000,
        irm_base_lr=0.01,
        irm_base_opt="sgd",
    ...
    for seed in config.seeds:
        ...
        for step in range(config.steps):
            loss, train_nll, train_penalty, kl_loss = train_step(
                model, optimizer, flags, config, dp
            )
            ...
            print(
                f"seed={seed} step={step:02d} loss={loss:.6f} "
    ...
    mean_best = summary_df["best_test_acc"].mean()
    std_best = summary_df["best_test_acc"].std(ddof=0)
    print("\nsaved:", summary_path)
    print("saved:", history_path)
    print(
        f"\nPeak distribution: mean={100.0 * mean_best:.2f}% "
        f"std={100.0 * std_best:.2f}% "
