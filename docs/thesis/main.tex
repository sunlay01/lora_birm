...
本文首先在线性理论、参数自由度与 KL 项缩放三个层面说明: 低秩参数化并不能直接保证因果恢复, 但它能够压缩可训练自由度和贝叶斯化参数规模, 因而为大模型上的不变性约束提供了一个更可执行的工程化近似. 随后, 本文在 Colored MNIST、Spurious CIFAR 和 Qwen 三类平台上进行了实验验证. 结果表明: 在 CMNIST 上, 低秩贝叶斯路线能够在保持较好 OOD 精度的同时不增加训练代价; 在 Spurious CIFAR 上, 当前已归档的五种子快照结果在平均历史峰值上与 BIRM 的过程最佳均值基本持平, 但个别种子能够达到更高局部峰值, 且在相同诊断协议下以更短训练预算完成搜索; 在 Qwen 平台上, LoRA-BIRM Snapshot 在相近 time-to-best 的条件下记录到了最高的历史峰值, 说明受限底座适配与快照保留在大模型 shortcut 纠偏中具有可观测价值. 与此同时, 更保守的多种子与训练末端子群观察也显示, 当前方法在最差种子和最差环境稳定性上仍未完全解决. 因而, 本文最终主张是审慎的: LoRA-BIRM 当前最可靠的贡献, 是展示了低秩贝叶斯因果微调在高质量局部峰值搜索和训练预算压缩方面的潜力, 而不是证明其已在所有设置下稳定优于现有基线.
...
实验结果表明, 该框架在部分设置下能够以更低的训练成本接近全参数 BIRM 的过程最佳区间, 并在个别随机种子上达到更高局部峰值, 或在相近时间预算下达到更高峰值, 但其结论仍需结合具体任务、评估协议和随机种子稳定性来理解. 因而, 本文更希望把这些结果视为一种可复现的经验发现, 而不是对低秩因果学习的最终定论.
...
为保证正文数值可追溯, 本章表格中的均值、方差与耗时均直接对应仓库中的轻量结果文件 \path{cmnist/results/cmnist_ultimate_ablation_method_summary.csv}; 5 次独立运行的逐次明细保存在 \path{cmnist/results/cmnist_ultimate_ablation_seed_details.csv}. 后文涉及的百分点差值均由这两份文件中的原始数值四舍五入后得到.
...
因此, 本章将实验平台推进到 Spurious CIFAR 场景. 本章的目标不是简单追求单个最高测试分数, 而是从泛化精度、跨种子稳定性和训练预算三个维度考察方法在复杂视觉偏移中的实际表现. 结果显示, BIRM 相较 ERM 与 IRMv1 具有更稳健的 OOD 表现; 当前已归档的五种子 LoRA-BIRM 快照结果在平均历史峰值上与 BIRM 的过程最佳均值基本持平, 但个别种子能够达到更高局部峰值, 同时在当前快照搜索协议下将平均单种子运行时间从 $345.68$s 压缩到 $128.78$s. 但同一组结果也暴露出一个重要限制: 训练末端平均精度仅约 $51.20\%$, 最差种子的历史峰值为 $52.90\%$, 说明当前低秩搜索策略仍不能保证所有随机种子和所有环境子群都稳定学好. 因而, 本章给出的结论是审慎的: 该方法当前最能成立的卖点, 是\textbf{以较短训练预算搜索并保留高质量局部 OOD 峰值}, 而距离完全稳健的 worst-group OOD 泛化仍需要进一步改进. 为避免把实验设置、基线复现、方法设计和失败模式混在一起, 本章按照“实验设置与评估口径 $\rightarrow$ 完整训练基线 $\rightarrow$ 低秩快照设计 $\rightarrow$ 主结果 $\rightarrow$ 稳定性边界”的顺序展开.
...
为了避免数据划分或随机初始化造成偶然结论, 本章对 ERM、IRMv1 与 BIRM 三个基线分别运行 5 个随机种子, 即 $11,17,23,29,37$. 每个种子记录训练过程中每 100 步的测试准确率以及最终测试准确率. 对本文方法, 本章当前主报告的是一组已归档的 5 种子低秩快照结果, 并通过最佳快照、最佳步数与训练末端精度来共同观察其稳定性边界.
...
为保证正文数值可追溯, 本章只将已经归档到仓库的两组轻量结果文件作为主证据: ERM、IRMv1 与 BIRM 的 5 种子主基线来自 \path{cifar/results/official_cifarmnist_main_method_summary.csv} 与 \path{cifar/results/official_cifarmnist_main_seed_details.csv}; LoRA-BIRM 快照五种子结果来自 \path{cifar/results/official_cifarmnist_thesis312_snapshot_method_summary.csv} 与 \path{cifar/results/official_cifarmnist_thesis312_snapshot_seed_details.csv}. 因此, 后文不再把未归档的早期筛选日志作为正文主证据, 而只保留能够直接回溯到结果文件的数值.
...
\section{低秩快照实验: 五种子结果、局部高峰与预算折中}
\subsection{已归档的五种子快照主结果}

当前仓库中已归档的 LoRA-BIRM 快照主结果来自 \path{cifar/results/official_cifarmnist_thesis312_snapshot_method_summary.csv} 与 \path{cifar/results/official_cifarmnist_thesis312_snapshot_seed_details.csv}. 基于这组可直接回溯的结果, 表~\ref{tab:cifar_lora_snapshot} 汇报了 IRMv1 $\rightarrow$ LoRA-BIRM 在 5 个随机种子上的历史最佳快照表现.

\begin{table}[htbp]
    \centering
    \caption{已归档的 IRMv1 $\rightarrow$ LoRA-BIRM 五种子快照主结果.}
    \label{tab:cifar_lora_snapshot}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{lccccc}
        \toprule
        方法 & 平均快照精度(\%) & 标准差(\%) & 最差种子(\%) & 平均运行时间(s) & 平均最佳步数 \\
        \midrule
        IRMv1 $\rightarrow$ LoRA-BIRM & 59.22 & 4.60 & 52.90 & 128.78 & 329.8 \\
        \bottomrule
    \end{tabular}
    }
\end{table}

从均值看, 该配置的平均历史峰值为 $59.22\%$, 与 BIRM 的过程最佳均值 $59.34\%$ 基本持平. 这说明在当前已归档的五种子口径下, LoRA-BIRM 还不能被表述为“已经稳定超过 BIRM”. 但从预算看, 其平均单种子运行时间为 $128.78$s, 相比 BIRM 的 $345.68$s 减少了约 $62.7\%$, 说明低秩快照式搜索确实在当前诊断协议下压缩了训练预算.

另一方面, 其最差种子的历史峰值仅为 $52.90\%$, 仍低于 BIRM 的最差最佳精度 $56.50\%$. 因而, 这组五种子结果更适合支持如下判断: 低秩快照路线已经能够以更短预算进入接近 BIRM 高点的区间, 但还没有把这种高点稳定转化为所有随机种子下都能守住的优势.

\subsection{种子级别高峰与训练末端回落}

仅看均值仍不足以揭示该方法的行为模式. 表~\ref{tab:cifar_lora_seed_table} 给出了 5 个随机种子的最佳快照、最佳步数与训练末端表现. 这些数据表明, LoRA-BIRM 的主要特征不是“末端稳定收敛”, 而是“在训练过程中搜索并捕获局部高峰”.

\begin{table}[htbp]
    \centering
    \caption{IRMv1 $\rightarrow$ LoRA-BIRM 五种子快照明细.}
    \label{tab:cifar_lora_seed_table}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{cccccc}
        \toprule
        种子 & 最佳快照精度(\%) & 最佳步数 & 快照标签 & 运行时间(s) & 训练末端精度(\%) \\
        \midrule
        11 & 65.80 & 350 & current & 127.15 & 49.80 \\
        17 & 57.40 & 300 & ema & 129.43 & 50.10 \\
        23 & 63.00 & 300 & ema & 128.53 & 50.00 \\
        29 & 52.90 & 100 & current & 129.04 & 49.10 \\
        37 & 57.00 & 599 & current & 129.74 & 57.00 \\
        \bottomrule
    \end{tabular}
    }
\end{table}

从表中可以看到, seed 11 与 seed 23 的最佳快照分别达到 $65.80\%$ 与 $63.00\%$, 甚至高于 BIRM 五种子中观察到的最高过程最佳值 $62.60\%$. 这说明低秩快照空间并非缺乏表达能力, 它确实能够在某些初始化与采样轨迹下找到相当强的 OOD 局部解. 但另一方面, seed 29 的最佳值只有 $52.90\%$, 而且 5 个种子的训练末端平均精度仅约为 $51.20\%$, 其中多数运行在训练后半程都回落到接近随机水平. 因而, 这组结果更支持“局部高峰搜索能力”这一卖点, 而不是“训练越久越稳、均值已经全面改写基线”这样的表述.

\subsection{预算压缩与精度折中}

除了 OOD 精度之外, 本章另一个关键观察是: 在当前快照搜索协议下, 低秩快照式 LoRA-BIRM 仍表现出可观测的时间压缩. 表~\ref{tab:cifar_runtime} 给出了 ERM、IRMv1、BIRM 与已归档五种子快照配置的平均单种子运行时间. 可以看到, LoRA-BIRM Snapshot 的平均运行时间为 $128.78$s, 相比 ERM 和 IRMv1 分别约快 $2.44\times$ 与 $2.49\times$, 相比 BIRM 约快 $2.68\times$.

\begin{table}[htbp]
    \centering
    \caption{Spurious CIFAR 上不同方法的平均单种子运行时间与相对 BIRM 加速比.}
    ...
    \begin{tabular}{lcc}
        ...
        IRMv1 $\rightarrow$ LoRA-BIRM Snapshot & \textbf{128.78} & \textbf{2.68$\times$} \\
        \bottomrule
    \end{tabular}
\end{table}

这一时间压缩并不能简单归因于“LoRA 单步计算更快”. 更准确地说, 它来自三部分机制的叠加. 第一, 低秩参数化冻结了大部分骨干参数, 将可训练自由度限制在 LoRA 子空间中, 降低了优化搜索的复杂度. 第二, 快照式模型选择避免了在后期继续消耗大量无效训练预算; 前文已经看到, 复杂场景下的长训练并不一定带来更稳的泛化, 反而可能破坏早期高质量 OOD 解. 第三, 低秩快照实验将训练目标从“等待最终收敛”改为“在受限预算内捕获最佳当前模型或 EMA 模型”. 因此, 这里更适合被理解为\textbf{当前过程最佳诊断协议下的预算压缩}, 而不是单独由参数化形式带来的严格复杂度结论.

从预算--精度折中看, IRMv1 $\rightarrow$ LoRA-BIRM 的平均历史峰值为 $59.22\%$, 与 BIRM 的过程最佳均值 $59.34\%$ 非常接近, 但平均运行时间从 $345.68$s 降至 $128.78$s. 更合理的理解方式是: 本文方法的优势并不主要体现为“最终稳定均值全面改写基线”, 而是体现为\textbf{能够在较短预算内把搜索轨迹推进到接近甚至局部超过 BIRM 的峰值区间, 并通过快照保留机制把这些高点捕获下来}. 这一点对于后续向更大 CNN、视觉 Transformer 甚至基础模型迁移具有工程参考价值.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/cifar_snapshot_story}
    \caption{Spurious CIFAR 上 LoRA-BIRM Snapshot 的预算--峰值折中与训练末端回落. 左图比较四种方法的平均运行时间与平均历史最佳 OOD 精度; 右图展示 IRMv1 $\rightarrow$ LoRA-BIRM 在 5 个种子上的最佳快照与训练末端精度差距, 并以 BIRM 的过程最佳均值与单种子最高值作为参考线.}
    \label{fig:cifar_snapshot_story}
\end{figure}

图~\ref{fig:cifar_snapshot_story} 把本章的核心叙事压缩到了同一张图中. 左图显示, LoRA-BIRM Snapshot 在平均历史峰值几乎不低于 BIRM 的情况下显著缩短了运行时间; 右图则显示, 这种优势主要来自训练过程中的局部高点而非训练末端稳定收敛, 多数种子都存在明显的 best--final gap, 只有 seed 37 在训练末端基本保住了快照峰值. 因而, 这张图同时支持了“预算压缩”和“快照必要性”两条结论.

\section{稳定性边界与失败模式}
\subsection{平均精度掩盖的最差环境问题}

本章实验最重要的发现并不是某一配置取得了最高均值, 而是历史高点与训练末端之间存在明显裂缝. 在已归档的五种子结果中, 历史最佳快照均值为 $59.22\%$, 但训练末端平均精度仅约 $51.20\%$; 与之对应, 训练末端的 \texttt{final\_ood\_minacc\_pct} 与 \texttt{final\_ood\_majacc\_pct} 均值分别只有约 $51.29\%$ 与 $50.40\%$. 这说明即使模型在中途曾找到较好的 OOD 解, 若不通过快照保留机制及时截获, 后续训练仍可能把模型重新拉回接近随机水平.
...
低秩 LoRA 子空间一方面提供了容量瓶颈, 抑制模型记忆高维虚假噪声; 另一方面也限制了优化路径的可达性. 当随机初始化与 mini-batch 序列恰好引导低秩方向对齐因果特征时, 模型可以在中途出现较高 OOD 快照; 但当低秩方向偏向某个环境特有模式时, 模型又很容易停留在 $50\%$ 左右. 这解释了为什么同样的五种子设置下, seed 11 可以达到 $65.80\%$, seed 23 可以达到 $63.00\%$, 而 seed 29 的最佳值却只有 $52.90\%$, 且其训练末端又回落到 $49.10\%$.

此外, EMA 快照虽然能够保存早期高点, 但它本质上是一种模型选择机制, 不是不变性学习机制本身. 如果训练目标没有显式优化最差环境, 那么最佳快照仍可能只是偶然出现的局部泛化高点. 当前已归档结果中, 多个种子的训练末端 \texttt{final\_ood\_minacc\_pct} 仍徘徊在 $50\%$ 左右, 说明稳定性并没有真正被抬起来. 因此, 本章低秩快照实验的价值在于定位问题: LoRA-BIRM 的低秩空间并非没有表达能力, 但现有 objective 对最差环境的约束仍不充分.
...
第三, \textbf{LoRA-BIRM 的更强卖点是“局部高峰搜索能力”, 而不是已经稳定超越 BIRM}. 在当前已归档的五种子快照结果中, IRMv1 $\rightarrow$ LoRA-BIRM 的平均历史峰值为 $59.22\%$, 与 BIRM 的 $59.34\%$ 基本持平; 但 seed 11 与 seed 23 分别达到 $65.80\%$ 与 $63.00\%$, 说明低秩快照空间确实能够在部分初始化下找到更高局部峰值. 与此同时, 其最差种子仅为 $52.90\%$, 训练末端平均精度也只有约 $51.20\%$, 因而当前阶段更合理的说法仍然是: 该方法擅长搜索并保留局部高点, 但最差种子和最差环境稳定性仍未解决.

第四, \textbf{在当前过程最佳诊断协议下, 低秩快照式搜索带来了可观测的训练预算压缩}. IRMv1 $\rightarrow$ LoRA-BIRM 的平均单种子运行时间为 $128.78$s, 相比 BIRM 的 $345.68$s 对应约 $2.68\times$ 的加速或 $62.7\%$ 的时间下降. 因而, 即使稳定性仍需改进, 本文方法已经在复杂场景中展示了可观测的工程扩展潜力.
...
为保证正文数值可追溯, 本章四种方法的峰值时间与准确率直接对应 \path{qwen/results/qwen_erm_irmv1_birm_baseline_results.csv}、\path{qwen/results/qwen_lora_birm_stable_400_lr2e-5_pen3_ema98/summary.csv} 与 \path{qwen/results/qwen_lora_birm_stable_400_lr2e-5_pen3_ema98/step_history.csv}. 因而, 后文所有 checkpoint 数值都可直接回溯到这三份文件.
...
在理论与方法层面, 本文并未试图给出一个过满的严格保证, 而是建立了一条更保守的论证链条: 低秩参数化能够压缩可训练自由度, 小参数子空间上的贝叶斯化能够降低 KL 项的规模压力, 因而为“不变性约束如何进入大模型”提供了合理的工程化近似. 在实验层面, CMNIST 结果说明低秩贝叶斯路线在最小可控平台上是可行的; Spurious CIFAR 结果说明该方法更强的卖点不是已经稳定超过 BIRM, 而是能够在五种子快照结果中以更短预算接近 BIRM 的过程最佳区间, 并在个别种子上达到更高局部峰值; Qwen 结果则进一步表明, 当允许有限的高层低秩适配并结合快照保留时, 方法能够在相近 time-to-best 的条件下取得更高的历史峰值.
