...
在工程实证层面, 本文基于标准的分布外(OOD)泛化任务构建了对比实验. 这些实验覆盖了从受控玩具数据到更复杂视觉场景, 再到 Qwen 平台上的文本 shortcut 设置, 用于检验低秩参数化与贝叶斯不变性约束结合后是否能够带来可观测的泛化--效率折中收益. 

实验结果表明, 该框架在部分设置下能够以更低的训练成本取得与全参数基线相近或略优的 OOD 表现, 但其结论仍需结合具体任务、评估协议和随机种子稳定性来理解. 因而, 本文更希望把这些结果视为一种可复现的经验发现, 而不是对低秩因果学习的最终定论.
...
因此, 本章将实验平台推进到 Spurious CIFAR 场景. 本章的目标不是简单追求单个最高测试分数, 而是从泛化精度、跨种子稳定性和训练预算三个维度考察方法在复杂视觉偏移中的实际表现. 结果显示, BIRM 相较 ERM 与 IRMv1 具有更稳健的 OOD 表现; 本文提出的低秩快照式 LoRA-BIRM 在平均精度上接近 BIRM 的过程最佳区间, 同时在当前快照搜索协议下压缩了单种子运行时间. 但实验也暴露出一个重要限制: 当前低秩搜索策略仍不能保证所有随机种子和所有环境子群都稳定学好. 因而, 本章给出的结论是审慎的: 该方法在复杂场景中展示了可行性和预算压缩潜力, 但距离完全稳健的 worst-group OOD 泛化仍需要进一步改进.
...
短预算先导实验用于回答一个基础问题: 在自然图像的复杂虚假相关中, 低秩贝叶斯子空间是否能够在较少训练步数内找到有效 OOD 解. 表~\ref{tab:cifar_lora_pilot} 汇报了三种代表性低秩搜索配置. 其中, \textbf{Soft-Group Snapshot} 使用软分组环境目标与 EMA 快照选择, 取得最高平均精度 $60.93\%$, 略高于 BIRM 的过程最佳均值 $59.34\%$. 但其最差种子仅为 $50.40\%$, 标准差达到 $9.59\%$, 说明该结果仍主要来自个别种子的高峰表现.
...
五种子稳定性实验揭示了短预算先导结果背后的不稳定性. 当种子数从 3 扩展到 5 后, 最优配置的均值从 $60.93\%$ 回落到 $59.22\%$, 仍高于 ERM 和 IRMv1 的最终表现, 但与 BIRM 的过程最佳均值基本持平. 这说明低秩贝叶斯快照策略并非完全无效, 但其最差种子表现仍不能支撑“对所有环境都学好”的强结论.
...
从预算--精度折中看, Stable Snapshot (120 steps) 在五种子上的平均 OOD 精度为 $59.22\%$, 与 BIRM 的过程最佳均值 $59.34\%$ 非常接近, 但平均运行时间从 $345.68$s 降至 $22.11$s, 对应约 $93.6\%$ 的时间下降. 因此, 即使当前方法在最差种子和最差环境稳定性上尚未完全超过 BIRM, 它仍说明低秩快照式因果微调有可能在复杂视觉分布偏移中以更短训练预算逼近 BIRM 的历史峰值区间. 这一点对于后续向更大 CNN、视觉 Transformer 甚至基础模型迁移具有工程参考价值.
